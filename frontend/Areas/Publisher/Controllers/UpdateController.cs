...
namespace Localactors.webapp.Areas.Publisher.Controllers
{
    ...
    [ValidateInput(false)]
    public class UpdateController : Localactors.webapp.Areas.Publisher.ControllerBase
    {
        ...
        {
            if (projectid <= 0) return RedirectToAction("Index", "Home");
        ...
        public ActionResult DeleteConfirmed(int id, int projectid = 0)
        {
            ...
            return RedirectToAction("Edit", "Projects", new { id = update.ProjectID }, "updates"); 
        ...
        public ActionResult ContentCreate(update_content update_content, int ProjectID = 0) {
            ...
            var update = db.updates.FirstOrDefault(x => x.UpdateID == update_content.UpdateID);
            var project = db.projects.FirstOrDefault(x => x.ProjectID == update.ProjectID && x.UserID == CurrentUser.UserID);
            if (project == null)
                return RedirectToAction("Index", "Home");
